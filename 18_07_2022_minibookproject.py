...
class Account:
    ...
    # Define a __init__ constructor whith attributes shared all by accounts
    def __init__(self,acc_number,opeaning_balance):
        self.acc_number = acc_number
        self.balance = opeaning_balance
    
    # Define a __str__ method to display attributes for accounts
    def __str__(self):
...
class checkingAccount(Account):
    ...
    def __init__(self, acc_number, opeaning_deposit):
        super().__init__(acc_number, opeaning_deposit)

    # Define the str mehtod which return the bank statement
    def __str__(self):
...
class Cusotmer:
    ...
    def __init__(self,name,pin):
        self.name = name
        self.pin = pin
        self.accounts = {'CHECKING':[],'SAVING':[],'BUSINESS':[]}

    def __str__(self):
        return self.name

    def open_checking(self,acc_number,opeaning_deposit):
        self.accounts['CHECKING'].append(Checking(acc_number,opeaning_deposit))   

    def open_saving(self,acc_number,opeaning_deposit):
        self.accounts['SAVING'].append(Saving(acc_number,opeaning_deposit))

    def open_business(self,acc_number,opeaning_deposit):
        self.accounts['BUSINESS'].append(Business(acc_number,opeaning_deposit))


    def get_total_deposit(self):
        total = 0

        for accnt  in  self.accounts['CHECKING']:
            print(accnt)
            total += accnt.balance
        for accnt in self.accounts['SAVING']:
            print(accnt)
            total += accnt.balance
        for accnt in self.accounts['BUSINESS']:
            total += accnt.balance

        print(f'Combined deposits in all accounts is :${total:.2f}\n')

def make_deposit(cust , acc_type,accnt_nbr,dep_amnt):
    for accnt in cust.accounts[acc_type]:
        if accnt_nbr == accnt.acc_number:
    ...
    while True:
        ...
        while True and select ==1:
            ...
            if option == 3:
                break
            if option ==1:
                exist_custm = int(input('Are you an existing cusotmer ? \n1. Yes\n2. no\n'))
            ...
            try:
                ...
                acc_type = input('Please enter the type of account you want to open With write :\nChecking: "CHECKING", Saving: "SAVING", Business: "BUSINESS"\n')
                if option == 2:
                ...
                if option == 2:
                    ...
                    elif acc_type.upper() == 'SAVING':
                        customers.open_saving(next(account_nbr), opening_deposit)
                    elif acc_type.upper() == 'BUSINESS':
